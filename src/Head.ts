...
export class Head extends Node {
  ...
  public serpentine(dt: number) {
    ...
    timer += dt
    const oldPos = this.getComponent(Transform).position
    const oscilatingY = oldPos.y + AMPLITUDE * Math.sin(timer * FREQUENCY)
    const newPos = new Vector3(oldPos.x, oscilatingY, oldPos.z)
    //this.addComponent(
  ...
  public getPathTime(value: number) {
    ...
    const percent = position / 64
    const percentToComplete = 1 - percent
    const currentTimeToCompletePath = this.velocity * percentToComplete
    // log(value)
