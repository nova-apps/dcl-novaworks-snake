...
export class UI {
  ...
  public shiftControls() {
    ...
    input.subscribe('BUTTON_DOWN', ActionButton.FORWARD, false, () => {
      if (toggle) {
        this.snake.head.forward()
      }
    })

    input.subscribe('BUTTON_DOWN', ActionButton.BACKWARD, false, () => {
      if (toggle) {
        this.snake.head.backward()
      }
    })

    input.subscribe('BUTTON_DOWN', ActionButton.LEFT, false, () => {
      if (toggle) {
        this.snake.head.turnLeft()
      }
    })

    input.subscribe('BUTTON_DOWN', ActionButton.RIGHT, false, () => {
      if (toggle) {
